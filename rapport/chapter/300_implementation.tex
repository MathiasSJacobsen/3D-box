...
\section{Svelte}
In this project, to implement the application we use Svelte. Svelte is a JavaScript framework used for building user interfaces, just like React, Angular, Vue, etc. Where some of these frameworks bulk there work in the browser, Svelte shifts that work onto a compile step~\cite{sveltewebsite}. Svelte is written using TypeScript. Instead of using virtual \gls{dom} Svelte uses build time to covert the code into JavaScript~\cite{sveltedocs}. 
